...
resource "aws_ecs_task_definition" "sql-task" {
  family                   = "snipeit-sql"
...
resource "aws_ecs_service" "service-my-sql" {
  name            = "service-snipeit-sql"
  cluster         = aws_ecs_cluster.foo.id
  task_definition = aws_ecs_task_definition.sql-task.arn
  desired_count   = 1
  launch_type     = "FARGATE"
